...
const ChatMessage = ({ message }: ChatMessageProps) => {
  ...
  return (
    ...
    >
      <div
        className={`flex gap-3 max-w-[75%] items-start ${
      ...
      >
        <div
          className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center border ${
            isUser
              ? "bg-mit-red text-white border-mit-red"
        ...
        <div
          className={`py-3 px-4 rounded-xl text-sm leading-relaxed break-words max-w-full ${
            isUser
        ...
        >
          {message.imageUrl && (
            <img
              ...
              alt="上傳的圖片"
              className="max-w-full max-h-[120px] rounded-lg mt-2 mb-0 object-contain block shadow-md first:mt-0"
            />
          )}
              ...
              <div className="markdown-content prose prose-sm max-w-none">
                <ReactMarkdown
                  ...
                  components={{
                    h1: ({ node, ...props }) => (
                      <h1
                        className="text-xl font-bold mt-4 mb-2 first:mt-0 text-gray-800"
                        {...props}
                      />
                    ),
                    h2: ({ node, ...props }) => (
                      <h2
                        className="text-lg font-bold mt-4 mb-2 first:mt-0 text-gray-800"
                        {...props}
                      />
                    ),
                    h3: ({ node, ...props }) => (
                      <h3
                        className="text-base font-bold mt-3 mb-2 first:mt-0 text-gray-800"
                        {...props}
                      />
                    ),
                    h4: ({ node, ...props }) => (
                      <h4
                        className="text-sm font-bold mt-2 mb-1 first:mt-0 text-gray-800"
                        {...props}
                      />
                    ),
                    p: ({ node, ...props }) => (
                      <p className="mb-2 last:mb-0 text-gray-800" {...props} />
                    ),
                    ul: ({ node, ...props }) => (
                      <ul
                        className="list-disc mb-2 space-y-1.5 ml-4 text-gray-800"
                        {...props}
                      />
                    ),
                    ol: ({ node, ...props }) => (
                      <ol
                        className="list-decimal mb-2 space-y-1.5 ml-4 text-gray-800"
                        {...props}
                      />
                    ),
                    li: ({ node, ...props }) => (
                    ...
                    code: ({ node, inline, ...props }: any) =>
                      inline ? (
                        ...
                        <code
                          className="block bg-gray-100 p-2 rounded text-xs font-mono overflow-x-auto mb-2 text-gray-800"
                          {...props}
                        />
                      ),
